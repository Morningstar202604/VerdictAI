...
- M2 推理与知识深化：引用溯源 grounding（专家发言携带工具来源，前端渲染「来源」徽标）、裁决前完整性自检（未解释证据/未解决矛盾/缺法条引用 → 交人类复核）、反思节点（裁决前可证伪性审查，Reflexion）、侦查计划（复杂卷宗产出待证问题清单）、案例沉淀（结案裁决回填为类案供检索）、历史庭审复盘入口完善、知识库管理前端（新增/删除自定义条目）。
- M3 专业轮：bge-reranker 精排（可选依赖，缺失自动降级混合检索）、证据链强度评分与不确定性声明、用量统计与审计面板 + SSE 事件流、全流程可视化页、沙箱容器强化（默认断网 + 512MB/1 核/128 进程限额）。
- 语音输入：Web Speech API（zh-CN）快捷描述案情，浏览器不支持时提示降级。

## [0.8.0] - 2026-09-07
### Added
- 庭审阶段提示（有剧本的辩论）：第 2 轮强制交叉质证（点名回应他人主张+证据依据）、末轮结辩收束（一句话结论+证据清单+不抛新论点）；后端提示词与本地引擎陈述双端生效，实测 7/7 专家遵守阶段要求。
- 证据一键核验：`GET /api/cases/{id}/evidence-audit` 确定性体检（编号唯一/格式/描述完整/保管链/时间可解析）+ 案卷面板「🔍 一键核验」按钮与结果渲染。
- 裁决书导出：裁决工具条新增「📜 裁决书」——判决书式排版（事实/当事人/采信证据链/存疑/裁决主文/后续事项/落款），与含全文笔录的审判报告互补。
- 提示词审计链（`AUDIT_PROMPTS=true` 开启）：每次专家调用的系统提示/用户材料/响应原文落盘 `data/audit/{session}.jsonl`，供研究复现。

## [0.7.1] - 2026-09-07
### Fixed
- 全面检查（0.7.1 补丁）发现并修复断线容错的两处缺口：
  - **慢重连即作废**：断开后只有"新连接已接管"才保活，重连稍慢（真实浏览器退避 1 秒以上）辩论就被取消——改为脱离宽限期守护取消（`WS_DETACH_GRACE`，默认 120 秒，0=旧行为）：宽限期内 resume 续看且辩论继续（实测断开期间事件照常推进），超时无人认领才取消归档；
  - **人工介入/落槌队列断线即丢**：队列改为脱离期间保留、任务终结或新庭审开局才清空。
- 裁决质询的「真相推定」不再被硬截断为 90 字残句。
### Verified
- 0.7.0 三特性实测：并行（相邻专家时间 6 对重叠）、真流式（各专家 token 到达有持续跨度）、resume 协议（完成后重连正确回放全量历史）。
