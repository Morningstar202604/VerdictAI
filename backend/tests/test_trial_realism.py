"""0.8.0 新特性测试：庭审阶段提示（交叉质证/结辩）与证据一键核验端点。"""
...
def test_final_round_is_closing():
    h = _phase_hint(3, 3)
    assert "结辩" in h and "最终结论性意见" in h and "新论点" in h


def test_round1_free_exploration():
    assert _phase_hint(1, 3) == ""


def test_two_round_debate_has_no_cross_exam():
    # max_rounds=2 时第 2 轮即结辩，不设交叉质证
    assert _phase_hint(2, 2).startswith("（结辩轮）")
